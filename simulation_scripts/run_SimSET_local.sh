...
log=${DIR_OUTPUT}/conv_SimSET_projdata_to_STIR_modified.log
...
if [ ! -z "${weight_filename}" ]; then
  "/mnt/e/PET_project/PET_simulations_new/simulation_scripts/conv_SimSET_projdata_to_STIR_modified.sh" \
    "${weight_filename}" "${num_seg}" "${SCANNER}" > "${log}" 2>&1
fi
