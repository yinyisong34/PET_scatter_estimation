...
# Change this to the equvalent for you
...
for en1 in `count 0 $(($en1_bin_num - 1))` ;
    do for en2 in `count 0 $(($en2_bin_num - 1))` ; do
      ...
      case "$scatter_parameter" in
        3|8)
          datasets_per_energy=$(( num_scatter_bins * num_scatter_bins ))
          ;;
        6)
          datasets_per_energy=$(( num_scatter_bins + 1 ))
          ;;
        9|10)
          datasets_per_energy=$(( num_scatter_bins + 1 ))
          ;;
        *)
          datasets_per_energy=$num_scatter_bins
          ;;
      esac

      en_dataset=$(( en_bin_num * datasets_per_energy ))
  ...
  3|8) # scatter_parameter
    ...
    do for j in $all_scatter_bin_nums ;
      do 
         ...
         blue_pink_name="blue${i}_en${en1}_pink${j}_en${en2}"
         echo ${conv_cmdline} ${dataset} ${blue_pink_name}
         eval ${conv_cmdline} ${dataset} ${blue_pink_name}
      done 
    done 
    ...
    for i in $all_scatter_bin_nums
      do for j in $all_scatter_bin_nums
        do     
          k=$(( $i + $j )) 
          current=blue${i}_en${en1}_pink${j}_en${en2}.hs
